 ...
 *
 * PRICING POLICY (uniform 70% gross margin):
 *   retail = wholesale USD × 125 (৳/$) ÷ (1 − 0.70)
 * Gemini Pro's ৳460 retail matches this rule exactly ($1.10 → ৳460).
 * The same rule is applied to live ProdSeller products at runtime via
 * `src/lib/pricing.ts` (retailPriceBdt / retailPriceUsd), so the static
 * catalog, DB seed, and live API mapping never drift apart.
 */
